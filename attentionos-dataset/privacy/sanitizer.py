...
class PrivacySanitizer:
    ...
    def sanitize_text(self, text: str, ctx: dict[str, Any] | None = None) -> str:
        ...
        try:
            ...
            if ctx:
                for key in ("person", "doctor", "sender"):
                    ...
                    if val and isinstance(val, str) and len(val) > 1:
                        # Case insensitive replacement of explicit context name
                        sanitized = re.sub(
            ...
            # Fallback handling
            return _EMAIL_REGEX.sub("[EMAIL]", text)

    def sanitize_entities(
        self, entities: dict[str, Any], ctx: dict[str, Any] | None = None
    ) -> dict[str, Any]:
    ...
    def sanitize_record(
        self, record: dict[str, Any], ctx: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if "title" in sanitized_record:
                sanitized_record["title"] = self.sanitize_text(
                    str(sanitized_record["title"]), ctx=ctx
                )

            if "body" in sanitized_record:
                sanitized_record["body"] = self.sanitize_text(
                    str(sanitized_record["body"]), ctx=ctx
                )
            ...
            fallback_record["title"] = self.sanitize_text(str(record.get("title", "")))
            fallback_record["body"] = self.sanitize_text(str(record.get("body", "")))
