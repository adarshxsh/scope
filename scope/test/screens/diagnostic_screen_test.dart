...
class FakeGhostAnalysisEngine extends GhostAnalysisEngine {
  final bool returnFallback;

  FakeGhostAnalysisEngine({this.returnFallback = false});
...
void main() {
  group('DiagnosticScreen Widget Tests', () {
    ...
    testWidgets(
      'displays explicit Fallback status tag when isFallback is true',
      (WidgetTester tester) async {
        final fallbackEngine = FakeGhostAnalysisEngine(returnFallback: true);

        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(body: DiagnosticScreen(engine: fallbackEngine)),
          ),
        );
