...
class _AiPlaygroundScreenState extends State<AiPlaygroundScreen> {
    ...
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          final displayTitle = n.title.isEmpty
              ? '(No title)'
              : (_isPrivacyMode ? PiiRedactor.maskText(n.title) : n.title);
          final displayContent = _isPrivacyMode
              ? PiiRedactor.maskText(n.content)
              : n.content;
  ...
  Widget _buildPostMortemPanel(AppNotification n, ThemeData theme) {
    ...
    if (features.otp != null) {
      definingWords.add(_isPrivacyMode ? 'OTP:${PiiRedactor.maskOtp(features.otp)}' : 'OTP:${features.otp}');
    }
    if (features.amount != null) {
      definingWords.add(_isPrivacyMode ? 'Amount:${PiiRedactor.maskAmount(features.amount)}' : 'Amount:Rs.${features.amount}');
    }

    final rawTarget = '${n.title} - ${n.content}';
    final targetText = _isPrivacyMode ? PiiRedactor.maskText(rawTarget) : rawTarget;
    final explanationText = _isPrivacyMode
        ? PiiRedactor.maskText(n.explanation ?? 'Classified via heuristic rule matching.')
        ...
        children: [
          ...
          Wrap(
            ...
            children: definingWords.isEmpty
                ...
                : definingWords.map((w) {
                    final tagLabel = _isPrivacyMode ? PiiRedactor.maskText(w) : w;
                    return Chip(
                      label: Text(tagLabel, style: const TextStyle(fontSize: 11, color: Colors.white)),
          ...
          Text(explanationText, style: const TextStyle(fontStyle: FontStyle.italic, color: Colors.white70)),
