...
class LiteRtClassifier implements NotificationAnalyzer {
  ...
  Future<void> _initialize() async {
    try {
      // 1. Load Vocab via cryptographic AssetVerificationEngine
      final vocabStr = await AssetVerificationEngine.instance.loadAndVerifyString('assets/vocab.txt');
      if (vocabStr == null) {
        throw Exception('Asset verification failed for assets/vocab.txt');
      }
      final lines = vocabStr.split('\n');
      _tokenizer = WordPieceTokenizer.fromLines(lines);

      // 2. Load Model Buffer via cryptographic AssetVerificationEngine
      final modelBytes = await AssetVerificationEngine.instance.loadAndVerifyByteData('assets/model.tflite');
      if (modelBytes == null) {
        throw Exception('Asset verification failed for assets/model.tflite');
      }

      // 3. Instantiate TensorFlow Lite Interpreter from verified buffer
