...
class GhostAnalysisEngine {
  ...
  Future<AppNotification> analyze(AppNotification notification) async {
    ...
    try {
      ...
      stopwatch.stop();
      if (kDebugMode) {
        debugPrint('GhostAnalysisEngine.analyze isolated exception for pkg=${notification.packageName}: $e');
      }
      return notification.copyWith(
        priority: 'medium',
        priorityScore: 0.35,
        classifiedCategory: 'uncategorized',
        explanation: 'Analysis encountered an isolated error. Default fallback applied.',
        latencyMs: stopwatch.elapsedMilliseconds,
