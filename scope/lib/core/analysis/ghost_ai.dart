...
class GhostAI {
  ...
  Future<GhostAIResult> _predict(AppNotification notification) async {
    ...
    if (_interpreter != null) {
      try {
        ...
        final targetDim = (inputTensor.shape.length > 1 && inputTensor.shape[1] > 0)
            ? inputTensor.shape[1]
            : 63;
        final adaptedVector =
            FeatureVector(featureVector).padOrTruncate(targetDim).toList();
        ...
        if (rawScore.isFinite) {
          ...
          debugPrint(
            'GhostAI: Non-finite score from interpreter ($rawScore), falling back to heuristics.',
          );
          predictedScore = _heuristicLookAgainScore(featureVector);
        }
      } catch (e, stack) {
        debugPrint('GhostAI: Error during TFLite inference: $e\n$stack');
        predictedScore = _heuristicLookAgainScore(featureVector);
      }
    } else {
  ...
  /// Outputs structured AI execution reports in debug mode with cleartext PII redacted.
  void _logStructured(AppNotification notification, GhostAIResult result) {
    final titleRedacted = notification.title.replaceAll(
      RegExp(r'\b\d{4,8}\b'),
      '[REDACTED_OTP]',
    );
    final contentRedacted = notification.content
        .replaceAll(RegExp(r'\b\d{4,8}\b'), '[REDACTED_OTP]')
        .replaceAll(
          RegExp(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b'),
          '[REDACTED_EMAIL]',
        )
        .replaceAll(
          RegExp(r'(?:₹|rs\.?|inr|usd|\$|eur|€)\s*\d+(?:\.\d+)?', caseSensitive: false),
          '[REDACTED_AMOUNT]',
        );

    debugPrint('=== GHOST AI INFERENCE REPORT ===');
    debugPrint('Notification: "$titleRedacted" - "$contentRedacted"');
