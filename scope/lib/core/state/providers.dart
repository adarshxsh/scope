...
class AppSettingsNotifier extends StateNotifier<AppSettingsState> {
  ...
  Future<void> _loadSettings() async {
    if (_db == null) return;
    try {
      final entry = await _db!.appSettingsDao.getSettings();
  ...
  Future<void> setRetentionDays(int days) async {
    ...
    if (_db != null) {
      await _db!.appSettingsDao.updateSettings(retentionDays: days);
    }
  }

  Future<void> setStorageQuota(int count) async {
    await _loadFuture;
    state = state.copyWith(storageQuota: count);
    if (_db != null) {
      await _db!.appSettingsDao.updateSettings(storageQuota: count);
    }
  }
  ...
  Future<void> setTelemetryEnabled(bool enabled) async {
    ...
    if (_db != null) {
      await _db!.appSettingsDao.updateSettings(telemetryEnabled: enabled);
    }
  }
}
