...
/// Client-side Differential Privacy Budget Manager with isolated exception handling
/// and in-memory fallback state tracking.
class PrivacyBudgetManager {
  ...
  Future<PrivacyBudgetStatus> getStatus([DateTime? now]) async {
    ...
    if (dao != null) {
      try {
        ...
        spentToday = _getInMemorySpentToday(dateKey);
        spentMonth = _getInMemorySpentMonth(monthKey);
      }
    } else {
      spentToday = _getInMemorySpentToday(dateKey);
      spentMonth = _getInMemorySpentMonth(monthKey);
  ...
  double _getInMemorySpentToday(String dateKey) {
    if (_lastInMemoryDateKey != dateKey) {
      _inMemorySpentToday = 0.0;
      _lastInMemoryDateKey = dateKey;
    }
    return _inMemorySpentToday;
  }

  double _getInMemorySpentMonth(String monthKey) {
    if (_lastInMemoryMonthKey != monthKey) {
      _inMemorySpentMonth = 0.0;
      _lastInMemoryMonthKey = monthKey;
    }
    return _inMemorySpentMonth;
  }

  void _addInMemorySpent(String dateKey, String monthKey, double epsilon) {
    if (_lastInMemoryDateKey != dateKey) {
      _inMemorySpentToday = 0.0;
      _lastInMemoryDateKey = dateKey;
    }
    ...
    _inMemorySpentToday += epsilon;
    _inMemorySpentMonth += epsilon;
  }

  /// Record remote device epsilon consumption to sync budget state across instances.
  Future<void> recordRemoteConsumption({
    ...
    if (dao != null) {
      try {
        await dao.recordRemoteConsumption(date, epsilon, delta);
      } catch (_) {
        _addInMemorySpent(date, monthKey, epsilon);
      }
    } else {
      _addInMemorySpent(date, monthKey, epsilon);
    }
  ...
  Future<NoisedQueryResult<T>> executeNoisedQuery<T extends num>({
    ...
    // Record privacy budget consumption in persistent ledger with fallback
    final dao = _ledgerDao;
    if (dao != null) {
      try {
