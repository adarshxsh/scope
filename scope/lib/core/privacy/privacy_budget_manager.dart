...
class LaplaceNoise {
  /// Generates a Laplace noise sample with scale parameter $b = \text{sensitivity} / \epsilon$.
  ...
  /// Calculates scale $b = \Delta f / \epsilon$.
...
class PrivacyBudgetManager {
  ...
  PrivacyLedgerDao? get _ledgerDao => _db?.privacyLedgerDao;
  ...
  /// Fetches current privacy budget status from the persistent ledger.
  Future<PrivacyBudgetStatus> getStatus([DateTime? now]) async {
    ...
    if (_ledgerDao != null) {
      spentToday = await _ledgerDao!.getEpsilonSpentForDate(dateKey);
      spentMonth = await _ledgerDao!.getEpsilonSpentForMonth(monthKey);
    }
  ...
  Future<NoisedQueryResult<T>> executeNoisedQuery<T extends num>({
    ...
    if (status.remainingDaily < qEpsilon || status.remainingMonthly < qEpsilon) {
      ...
      final lower = (val / coarseningBucket).floor() * coarseningBucket.toInt();
      final upper = lower + coarseningBucket.toInt();
    ...
    if (_ledgerDao != null) {
      await _ledgerDao!.recordQueryConsumption(dateKey, qEpsilon, delta);
    }
  ...
  Future<List<NoisedQueryResult<num>>> executeVectorQuery({
    ...
    if (_ledgerDao != null) {
      await _ledgerDao!.recordQueryConsumption(dateKey, totalEps, 0.0);
    }
