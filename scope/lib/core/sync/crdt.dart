...
/// Differential Privacy Budget CRDT delta payload for cross-device telemetry budget synchronization.
class PrivacyBudgetDelta {
  final String originDeviceId;
  ...
  const PrivacyBudgetDelta({
    required this.originDeviceId,
    required this.date,
    required this.epsilonConsumed,
    this.deltaConsumed = 0.0,
    this.queryCount = 1,
    required this.vectorClock,
    required this.timestamp,
  });

  factory PrivacyBudgetDelta.fromMap(Map<String, dynamic> map) {
    return PrivacyBudgetDelta(
      originDeviceId: map['origin_device_id'] as String? ?? '',
      date: map['date'] as String? ?? '',
      epsilonConsumed: (map['epsilon_consumed'] as num? ?? 0.0).toDouble(),
      deltaConsumed: (map['delta_consumed'] as num? ?? 0.0).toDouble(),
      queryCount: (map['query_count'] as num? ?? 1).toInt(),
      vectorClock: VectorClock.fromMap(
        Map<String, dynamic>.from(map['vector_clock'] as Map? ?? {}),
      ),
      timestamp: map['timestamp'] as int? ?? 0,
  ...
  factory PrivacyBudgetDelta.fromJson(String jsonStr) {
    return PrivacyBudgetDelta.fromMap(json.decode(jsonStr) as Map<String, dynamic>);
  }
