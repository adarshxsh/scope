...
/// with memory quota bounds and automated rotation.
class FeedbackDatasetLogger {
  ...
  /// Resolves the log file handle safely.
  ...
  /// Appends a user feedback interaction to local JSONL dataset storage with quota checks.
  Future<bool> logFeedback({
    ...
    try {
      ...
      // Extract feature vector (63 features)
      ...
      // Perform quota rotation check
      await _rotateIfNeeded(file);

      debugPrint('FeedbackDatasetLogger: Persisted feedback record (type: $feedbackType).');
      return true;
    } catch (e) {
  ...
  /// Reads all valid feedback records from storage.
