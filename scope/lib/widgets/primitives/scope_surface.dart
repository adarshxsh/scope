...
class ScopeSurface extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget surface = switch (variant) {
      ScopeSurfaceVariant.glass || ScopeSurfaceVariant.glassDark => _GlassLayer(
        padding: pad,
        radius: radius,
    ...
    if (onTap != null && animatePress && variant == ScopeSurfaceVariant.solid) {
      return MotionSurface(onTap: onTap!, borderRadius: radius, child: surface);
    }
...
class _GlassLayer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (borderColor != null) {
      fill = Color.alphaBlend(borderColor!.withValues(alpha: 0.15), fill);
    }
    final border =
        borderColor ??
        (dark ? AppColors.onSurface.withValues(alpha: 0.08) : AppColors.border);
