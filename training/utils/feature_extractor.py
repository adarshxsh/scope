...
def extract_features(record: dict[str, Any]) -> list[float]:
    ...
    feature_map = {
        "title_length": float(len(title_norm)),
        "body_length": float(len(body_norm)),
        "word_count": float(len(_wordRegex.findall(combined))),
        "uppercase_ratio": 0.0 if letters == 0 else float(uppercase / letters),
        "digit_ratio": float(digits / text_unit_count),
        "emoji_count": float(len(_emojiRegex.findall(combined))),
        "punctuation_count": float(len(_punctuationRegex.findall(combined))),
        "contains_question": 1.0 if '?' in combined else 0.0,
        "contains_exclamation": 1.0 if '!' in combined else 0.0,
        "contains_currency_symbol": 1.0 if _currencySymbolRegex.search(combined) else 0.0,
        "contains_money": 1.0 if (_amountRegex.search(combined) or contains_keyword(lower, _moneyWords)) else 0.0,
        "contains_otp": 1.0 if otp is not None else 0.0,
        "contains_date": 1.0 if _dateRegex.search(combined) else 0.0,
        "contains_time": 1.0 if _timeRegex.search(combined) else 0.0,
        "contains_location": 1.0 if contains_keyword(lower, _locationWords) else 0.0,
        "contains_email": 1.0 if _emailRegex.search(combined) else 0.0,
        "contains_phone": 1.0 if _phoneRegex.search(combined) else 0.0,
        "contains_link": 1.0 if _urlRegex.search(combined) else 0.0,
        "contains_attachment": 1.0 if (contains_attachment or contains_any_phrase(lower, ['attachment', 'attached', 'file', 'photo', 'image', 'pdf', 'document'])) else 0.0,
        "contains_percentage": 1.0 if _percentageRegex.search(combined) else 0.0,
        "contains_discount": 1.0 if is_promotion else 0.0,
        "is_promotion": 1.0 if is_promotion else 0.0,
        "contains_order_id": 1.0 if contains_order else 0.0,
        "contains_transaction_id": 1.0 if contains_transaction else 0.0,
        "contains_reference_number": 1.0 if _referenceNumberRegex.search(combined) else 0.0,
        "contains_tracking_number": 1.0 if _trackingNumberRegex.search(combined) else 0.0,
        "contains_coupon": 1.0 if _couponRegex.search(combined) else 0.0,
        "contains_meeting_link": 1.0 if _meetingLinkRegex.search(combined) else 0.0,
        "contains_deadline": 1.0 if contains_deadline else 0.0,
        "contains_security_keywords": 1.0 if contains_keyword(lower, _securityWords) else 0.0,
        "contains_payment_keywords": 1.0 if contains_keyword(lower, _paymentWords) else 0.0,
        "contains_delivery_keywords": 1.0 if contains_keyword(lower, _deliveryWords) else 0.0,
        "contains_work_keywords": 1.0 if contains_keyword(lower, _workWords) else 0.0,
        "contains_social_keywords": 1.0 if contains_keyword(lower, _socialWords) else 0.0,
        "contains_system_keywords": 1.0 if contains_keyword(lower, _systemWords) else 0.0,
        "importance": float(importance),
        "conversation": 1.0 if conversation else 0.0,
        "visibility": float(visibility),
        "ongoing": 1.0 if ongoing else 0.0,
        "foreground_service": 1.0 if foreground_service else 0.0,
        "notification_category": float(android_category_id),
        "channel_id": float(stable_bucket(channel_id)),
        "channel_name": float(stable_bucket(channel_name)),
        "timestamp_hour": float(dt_utc.hour),
        "day_of_week": float(weekday),
        "requires_action": 1.0 if contains_keyword(lower, _actionWords) else 0.0,
        "is_recurring": 1.0 if contains_keyword(lower, _recurringWords) else 0.0,
        "is_duplicate_candidate": 1.0 if is_duplicate_candidate(lower) else 0.0,
        "deadline_exists": 1.0 if contains_deadline else 0.0,
        "deadline_minutes_remaining": float(deadline_minutes_remaining(lower)),
        "amount": float(amount),
        "currency": float(_currencyIds.get(currency, 0)),
        "otp_length": float(len(otp) if otp else 0),
        "merchant_present": 1.0 if (contains_keyword(lower, _merchantWords) or merchant_after_amount(combined)) else 0.0,
        "person_present": 1.0 if (contains_keyword(lower, _personWords) or looks_like_person_title(title)) else 0.0,
        "city_present": 1.0 if contains_keyword(lower, _cityWords) else 0.0,
        "train_present": 1.0 if contains_keyword(lower, _trainWords) else 0.0,
        "flight_present": 1.0 if contains_keyword(lower, _flightWords) else 0.0,
        "order_present": 1.0 if contains_order else 0.0,
        "transaction_present": 1.0 if contains_transaction else 0.0,
        "intent_id": float(intent_id),
        "notification_type_id": float(notification_type_id),
